...
static void bluez_endpoint_select_configuration(GDBusMethodInvocation *inv, void *userdata) {
	...
	case A2DP_CODEC_VENDOR_APTX_HD: {
		...
		if (size != sizeof(a2dp_aptx_hd_t)) {
			error("Invalid capabilities size: %zu != %zu", size, sizeof(a2dp_aptx_hd_t));
			goto fail;
		}

		a2dp_aptx_hd_t *cap = (a2dp_aptx_hd_t *)capabilities;

		if (config.a2dp_force_44100 &&
				cap->frequency & APTX_HD_SAMPLING_FREQ_44100)
			cap->frequency = APTX_HD_SAMPLING_FREQ_44100;
		else if (cap->frequency & APTX_HD_SAMPLING_FREQ_48000)
			cap->frequency = APTX_HD_SAMPLING_FREQ_48000;
		else if (cap->frequency & APTX_HD_SAMPLING_FREQ_44100)
			cap->frequency = APTX_HD_SAMPLING_FREQ_44100;
		else if (cap->frequency & APTX_HD_SAMPLING_FREQ_32000)
			cap->frequency = APTX_HD_SAMPLING_FREQ_32000;
		else if (cap->frequency & APTX_HD_SAMPLING_FREQ_16000)
			cap->frequency = APTX_HD_SAMPLING_FREQ_16000;
		else {
			error("No supported sampling frequencies: %#x", cap->frequency);
			goto fail;
		}

		if (cap->channel_mode & APTX_HD_CHANNEL_MODE_STEREO)
			cap->channel_mode = APTX_HD_CHANNEL_MODE_STEREO;
		else {
			error("No supported channel modes: %#x", cap->channel_mode);
			goto fail;
		}
	...
	while (g_variant_iter_next(properties, "{&sv}", &key, &value)) {
		...
		if (strcmp(key, "Device") == 0) {
			...
			case A2DP_CODEC_VENDOR_APTX_HD: {
				...
				if (cap->frequency != APTX_HD_SAMPLING_FREQ_16000 &&
						cap->frequency != APTX_HD_SAMPLING_FREQ_32000 &&
						cap->frequency != APTX_HD_SAMPLING_FREQ_44100 &&
						cap->frequency != APTX_HD_SAMPLING_FREQ_48000) {
					error("Invalid configuration: %s", "Invalid sampling frequency");
...
#if ENABLE_APTX_HD
		bluez_register_a2dp_endpoint(BLUETOOTH_UUID_A2DP_SOURCE, BLUETOOTH_PROFILE_A2DP_SOURCE,
				A2DP_CODEC_VENDOR_APTX_HD, &bluez_a2dp_aptx_hd, sizeof(bluez_a2dp_aptx_hd));
#endif
#if ENABLE_APTX
		bluez_register_a2dp_endpoint(BLUETOOTH_UUID_A2DP_SOURCE, BLUETOOTH_PROFILE_A2DP_SOURCE,
